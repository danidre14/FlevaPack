...
const FPEngine = function (gameAreaId, options) {
    ...
    const objectInheritance = function (options, elem, type) {
        ...
        tempObject.defaultOD = options ? options.depth ? options.depth || 0 : 0 : 0;
        ...
        tempObject.DEPTH = tempObject.defaultOD;
        ...
        if (elem === "textarea") {
            tempObject.object = document.createElement("div");
            tempObject.object.contentEditable = "true";
            tempObject.object.style.overflow = "auto";
            tempObject.object.style.overflowWrap = "break-word";
        } else
        ...
        tempObject.object.style.zIndex = tempObject.DEPTH;
        ...
        if (!tempObject.SELECTION)
            tempObject.object.classList.add('gameObjectSelection');
        ...
        tempObject.object.setDepth = function (depth) {
            tempObject.DEPTH = depth !== undefined ? depth : tempObject.DEPTH;
            this.style.zIndex = tempObject.DEPTH;
        }
        tempObject.object.getDepth = function () {
            return tempObject.DEPTH;
        }
        ...
        if (options && options.visible === false) tempObject.object.style.display = "none";
    ...
    const createImage = function (name, ...rest) {
        ...
        gameobjects[name].object.setProperties = function (options) {
            ...
            if (options.depth) this.setDepth(options.depth);
        ...
        gameobjects[name].object.restore = function () {
            ...
            this.setDepth(gameobjects[name].defaultOD);
    ...
    const createTextfield = function (name, ...rest) {
        ...
        gameobjects[name].WIDTH = gameobjects[name].defaultOW;
        gameobjects[name].HEIGHT = gameobjects[name].defaultOH;
        gameobjects[name].RTEXT = gameobjects[name].defaultOT;
        ...
        gameobjects[name].object.innerHTML = gameobjects[name].RTEXT;
        ...
        if (options && options.selectable === false) {
            ...
            if (type === "textarea") gameobjects[name].object.contentEditable = false;
        }

        gameobjects[name].object.clearText = function () {
            gameobjects[name].RTEXT = "";
            gameobjects[name].TEXT = "";
            if (type === "textarea")
                this.innerHTML = gameobjects[name].RTEXT;
            else if (type === "input")
                this.value = gameobjects[name].TEXT;
        }

        const getComputedText = function (text, options) {
            text = text.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
            let modifiedText = text;
            if (options) {
                modifiedText = `<span style="`;
                if (options.color !== undefined && typeof options.color === "string" && options.color.trim() !== "") {
                    modifiedText += `color:${options.color};`
                }
                if (options.style !== undefined && typeof options.style === "string" && options.style.trim() !== "") {
                    for (const style of options.style.split(" "))
                        if (style === "italic" || style === "i")
                            modifiedText += `font-style: italic;`
                        else if (style === "bold" || style === "b")
                            modifiedText += `font-weight:bold;`
                }
                if (options.line !== undefined && typeof options.line === "string" && options.line.trim() !== "") {
                    modifiedText += "text-decoration:";
                    for (const line of options.line.split(" "))
                        if (line === "top")
                            modifiedText += " overline";
                        else if (line === "center")
                            modifiedText += " line-through";
                        else if (line === "bottom")
                            modifiedText += " underline";
                    modifiedText += ";";
                }
                if(options.link !== undefined && typeof options.link === "string" && options.link.trim() !== "") text = `<a href="${options.link}" style="all:inherit;" target="_blank">${text}</a>`;
                modifiedText += `">${text}</span>`;
            }
        ...
        gameobjects[name].object.addText = function (strings, options) {
            let text = typeof strings === "object" ? [...strings].join(" ") : strings;

            text = getComputedText(text, options);
            gameobjects[name].RTEXT += text !== undefined ? text : "";

            gameobjects[name].TEXT = getPlainText(gameobjects[name].RTEXT);
            if (type === "textarea")
                this.innerHTML = gameobjects[name].RTEXT;
            else if (type === "input")
                this.value = gameobjects[name].TEXT;
        }
        gameobjects[name].object.addTextLn = function (strings, options) {
            let text = typeof strings === "object" ? [...strings].join("\n") : strings;
            text = getComputedText(text, options) + "<br/>";
            gameobjects[name].RTEXT += text !== undefined ? text : "";
        ...
        gameobjects[name].object.removeTextLn = function (lines) {
            const text = gameobjects[name].RTEXT.split('<br/>').slice(0, -(Math.abs(lines) || 1)).join("<br/>");
            gameobjects[name].RTEXT = text !== undefined ? text : "";
            gameobjects[name].TEXT = getPlainText(gameobjects[name].RTEXT);
            if (type === "textarea")
                this.innerHTML = gameobjects[name].RTEXT;
        ...
        gameobjects[name].object.setText = function (text, options) {
            text = getComputedText(text, options);
            gameobjects[name].RTEXT = text !== undefined ? text : gameobjects[name].RTEXT;
            gameobjects[name].TEXT = getPlainText(gameobjects[name].RTEXT);
        ...
        gameobjects[name].object.getText = function () {
            return type === "input" ? this.value || "" : this.innerText || "";
        }
    ...
    const createViewport = function (name, ...rest) {
        ...
        viewports[name].object.restore = function () {
            ...
            this.setDepth(viewports[name].defaultOD);
            this.setAlign({ h: defaultOAH, v: defaultOAV, t: defaultOAT });
            this.setPosition({ h: defaultOPH, v: defaultOPV });
            resizeCanvas();
        }
